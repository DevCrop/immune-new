...
// // STATE 정의 (분기 경로 유지)
// $statePath = '/' . ltrim($branch, '/') . '/';

// // (권장) CSRF 방지용 랜덤 토큰을 state에 함께 포함
// if (empty($_SESSION['kakao_oauth_state_token'])) {
//     $_SESSION['kakao_oauth_state_token'] = bin2hex(random_bytes(16));
// }
// $stateToken = $_SESSION['kakao_oauth_state_token'];

// // REST API 설정
// $REST_API_KEY = "060e09813ff0141c60ad7e68b75c7805";

// // (TIP) 환경에 맞게 자동 구성: 개발/운영에서 모두 동작
// $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
// $host   = $_SERVER['HTTP_HOST'] ?? 'localhost:8080';
// $AUTH_REDIRECT_URL = "{$scheme}://{$host}/auth/kakao.php";

// // 스코프: 이메일을 받으려면 account_email 필수
// // 필요 시 프로필 스코프도 같이: profile_nickname, profile_image
// $scope = 'account_email profile_nickname profile_image';

// // state: 분기 경로 + CSRF 토큰을 함께 전달 (콜백에서 검증)
// $state = json_encode([
//     'path'  => $statePath,
//     'token' => $stateToken,
...
// $LOGIN_URL = "https://kauth.kakao.com/oauth/authorize"
//     . "?response_type=code"
//     . "&client_id=" . rawurlencode($REST_API_KEY)
//     . "&redirect_uri=" . rawurlencode($AUTH_REDIRECT_URL)
//     . "&scope=" . rawurlencode($scope)
//     . "&state=" . rawurlencode($state)
//     . "&prompt=login";  // 항상 카카오 로그인 화면 노출(선택)

// // 로그아웃 URL
// $LOGOUT_URL = "{$scheme}://{$host}/auth/logout.php";
